    ...
    <div class="max-w-7xl mx-auto w-full">
        ...
        <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-6 pb-6 border-b border-slate-200 gap-4">
            <div class="text-center md:text-left">
                ...
                <p class="text-xs md:text-sm text-slate-500 mt-1 font-medium">Manage store products and stock variants efficiently.</p>
            </div>
        ...
        <div class="hidden lg:block bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden mb-10">
            <table class="w-full border-collapse text-sm">
                <thead class="bg-[#0f172a] text-white text-left">
                    <tr>
                        <th class="py-4 px-6 font-black uppercase tracking-widest text-[10px] italic">Product / SKU</th>
                        ...
                        <th class="py-4 px-4 font-black uppercase tracking-widest text-[10px] italic text-right">Cost (TP)</th>
                        <th class="py-4 px-4 font-black uppercase tracking-widest text-[10px] italic text-right">Sale (MRP)</th>
...
<div id="productModal" class="fixed inset-0 bg-[#0f172a]/80 hidden flex justify-center items-center z-[100] p-2 sm:p-4 backdrop-blur-sm">
    <div id="productModalBox" class="bg-white w-full max-w-5xl rounded-3xl shadow-3xl flex flex-col overflow-hidden transform transition-all duration-300 scale-95 opacity-0 h-full max-h-[90vh]">
        <div class="p-5 border-b bg-[#0f172a] flex justify-between items-center">
            <h2 class="text-xl font-black text-[#f59e0b] italic uppercase tracking-tighter">Product Entry Hub</h2>
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 bg-[#f8fafc]">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Category / Spec</label>
                    <input id="p_generic" class="w-full px-4 py-3 border border-slate-200 rounded-2xl bg-slate-50 text-sm outline-none font-bold text-slate-600 uppercase" placeholder="e.g. Grocery, Electronics" />
                </div>
                <div>
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Brand / Manufacturer</label>
                    <input id="p_man" class="w-full px-4 py-3 border border-slate-200 rounded-2xl bg-slate-50 text-sm outline-none font-bold text-slate-600 uppercase" placeholder="e.g. Nestle, Unilever" />
                </div>
            </div>
                    ...
                    <table class="w-full text-xs min-w-[700px]">
                        <thead class="bg-slate-50 text-slate-400 font-black uppercase text-[9px]">
                            <tr class="text-left">
                                ...
                                <th class="p-4 text-right">Sale Price (MRP)</th>
        ...
        <div class="p-6 border-t bg-slate-50 flex flex-col md:flex-row justify-end gap-3">
            <button onclick="closeProductModal()" class="w-full md:w-auto px-8 py-3 bg-white border border-slate-200 rounded-xl font-black text-slate-400 uppercase text-xs hover:bg-slate-100 transition shadow-sm">Discard</button>
            <button id="saveProductBtn" class="w-full md:w-auto px-10 py-3 bg-[#0f172a] text-[#f59e0b] rounded-xl font-black shadow-xl uppercase text-xs tracking-widest hover:bg-slate-800 transition">Save Inventory</button>
        </div>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-[#0f172a]/80 hidden flex justify-center items-center z-[100] p-2 sm:p-4 backdrop-blur-sm">
    <div id="editModalBox" class="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-200 scale-95 opacity-0">
        ...
        <form id="editForm" class="p-5 md:p-8 space-y-6 bg-[#f8fafc]">
            ...
            <div class="space-y-4">
                <div>
                    <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Product Name</label>
...
<script>
    ...
    function updateVariantTable() {
        ...
        if (currentVariants.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" class="p-16 text-center text-slate-300 italic font-black uppercase tracking-widest text-[10px]">Add SKU to begin.</td></tr>';
            return;
        }

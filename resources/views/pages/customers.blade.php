...
@section('content')
<main class="h-screen overflow-y-auto p-2 sm:p-4 md:p-8 pt-24 bg-slate-50 flex flex-col">
    <div class="max-w-[1600px] mx-auto w-full">
        
        {{-- HEADER SECTION --}}
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 border-b border-slate-200 pb-6">
            <div>
                <h1 class="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 tracking-tighter italic uppercase pb-1 inline-block leading-none">
                    Customer Accounts
                </h1>
            ...
            <button onclick="openModalForAdd()"
                class="w-full md:w-auto px-6 py-4 bg-[#0f172a] text-white rounded-2xl shadow-xl hover:bg-slate-800 font-black uppercase text-xs tracking-widest transition active:scale-95 flex items-center justify-center gap-2">
        ...
        <div class="bg-white rounded-[30px] shadow-2xl border border-slate-100 p-4 sm:p-8">
            ...
            <div class="hidden lg:block overflow-hidden rounded-2xl border border-slate-100 shadow-sm">
                <table class="w-full border-collapse text-sm">
                    ...
                    <tbody id="customersTableBody" class="divide-y divide-slate-50 text-slate-800">
                        {{-- AJAX Data --}}
                    </tbody>
                </table>
            </div>
...
<div id="customerModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-[#0f172a]/80 backdrop-blur-sm transition-all">
    <div id="customerModalBox" class="bg-white w-full max-w-2xl rounded-[40px] shadow-3xl overflow-hidden flex flex-col opacity-0 scale-95 transition-all duration-300">
        <div class="bg-[#0f172a] px-8 py-6 flex justify-between items-center text-white shrink-0">
            <div>
                <h3 class="text-2xl font-black uppercase italic tracking-tighter leading-none text-amber-500" id="modalTitle">Customer Profile</h3>
                <p class="text-[10px] font-bold uppercase tracking-widest mt-1 text-slate-400">Registration Terminal</p>
            </div>
        ...
        <form id="customerForm" onsubmit="handleFormSubmit(event)" class="p-8 overflow-y-auto bg-slate-50">
            @csrf
            <div id="formAlerts" class="mb-6"></div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-1">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Customer Name *</label>
                    <input type="text" name="customer_name" id="f_name" required class="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl font-bold text-sm outline-none focus:border-amber-500 transition">
                </div>
                <div class="space-y-1">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Phone Number *</label>
                    <input type="text" name="phone_number" id="f_phone" required class="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl font-bold text-sm outline-none focus:border-amber-500 transition">
                </div>
                <div class="md:col-span-2 p-6 bg-slate-100 rounded-3xl border border-slate-200 shadow-inner">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block uppercase">Opening Balance (PKR)</label>
                    <input type="number" step="0.01" name="credit_balance" id="f_balance" value="0.00" class="w-full px-5 py-4 border-2 border-slate-200 rounded-2xl bg-white font-black text-2xl text-slate-900 italic outline-none focus:ring-8 focus:ring-amber-500/10 transition">
                </div>
            </div>
            <div class="flex flex-col sm:flex-row justify-end gap-3 mt-8 border-t border-slate-200 pt-6">
                <button type="button" onclick="closeCustomerModal()" class="px-8 py-3 bg-white border border-slate-200 rounded-2xl text-slate-400 font-black uppercase text-[10px] tracking-widest">Discard</button>
                <button type="submit" id="saveCustomerBtn" class="px-10 py-3 bg-[#0f172a] text-[#f59e0b] rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl hover:bg-slate-800 transition active:scale-95">Save Profile</button>
            </div>
        </form>
    </div>
</div>
...
{{-- MODAL: Ledger --}}
<div id="historyModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-[#0f172a]/80 backdrop-blur-md transition-all">
        ...
        <div class="bg-[#0f172a] px-8 py-6 flex justify-between items-center text-white shrink-0">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-slate-800 rounded-2xl flex items-center justify-center shadow-lg border border-slate-700">
                    <i class="fa-solid fa-file-invoice text-xl text-amber-500"></i>
                </div>
                <div>
                    <h2 id="historyCustomerName" class="text-2xl font-black italic tracking-tighter leading-none uppercase text-amber-500">Statement</h2>
            ...
            <button onclick="closeHistoryModal()" class="w-10 h-10 rounded-2xl bg-white/10 flex items-center justify-center hover:bg-white/20 transition text-2xl font-light">&times;</button>
        </div>

        <div class="flex flex-col lg:flex-row flex-grow overflow-hidden bg-slate-50">
            <div class="lg:w-2/3 flex-grow overflow-y-auto p-6 sm:p-8 border-r border-slate-200 custom-scrollbar bg-white">
            ...
            <div class="lg:w-1/3 p-6 sm:p-8 flex flex-col shrink-0 overflow-y-auto">
                <div class="bg-white p-6 rounded-[30px] shadow-sm border border-amber-100 mb-8 text-center ring-8 ring-amber-50/30">
                    <p class="text-[9px] font-black text-amber-600 uppercase tracking-widest mb-1">Outstanding Balance</p>

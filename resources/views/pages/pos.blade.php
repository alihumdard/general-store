...
@section('title', 'Retail POS - Point of Sale')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[2.5fr_1.5fr] gap-6 items-stretch">
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 h-[85vh] flex flex-col">
                <div class="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
                    <div>
                        <h2 class="text-2xl font-black text-slate-900 tracking-tighter uppercase leading-none">Product Catalog</h2>
                        <p class="text-[10px] text-amber-500 font-bold uppercase tracking-widest mt-1">Retail Inventory Management</p>
                    </div>
                    {{-- Search Bar --}}
                    <div class="relative w-full max-w-md">
                        <i class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                        <input id="productSearch" oninput="fetchProducts(1)"
                            class="w-full pl-11 pr-4 py-3 bg-gray-50 rounded-2xl border border-gray-200 focus:ring-4 focus:ring-amber-50 focus:border-amber-400 outline-none transition shadow-inner font-bold text-sm"
            ...
            <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 h-[85vh] flex flex-col overflow-hidden">
                <div class="border-b border-gray-100 pb-4 mb-4">
                    <h3 class="text-xl font-black text-slate-900 tracking-tighter uppercase leading-none">Store Receipt</h3>
                    <div class="mt-4 flex items-center gap-3">
                        <select id="customerSelect" class="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50 text-sm font-bold focus:ring-4 focus:ring-slate-50 outline-none">
                ...
                {{-- CALCULATION BOX --}}
                <div class="mt-auto space-y-3 bg-slate-50 p-5 rounded-3xl border border-slate-200 shadow-inner">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="text-[9px] font-black text-slate-400 uppercase ml-1">Srv. Charges</label>
                            <input type="number" id="serviceInput" oninput="calculateTotals()" value="0" class="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-slate-600 outline-none focus:ring-2 focus:ring-slate-100">
                        </div>
                    </div>

                    <div class="flex justify-between items-center pt-2 border-t border-slate-200 text-xs font-bold text-slate-500 uppercase font-black">
                        <span>Subtotal</span>
                        <span id="subtotal" class="text-slate-800">0.00</span>
                    </div>

                    <div class="flex justify-between items-end text-slate-900">
                    ...
                    <div class="pt-3 border-t border-slate-200">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 block mb-1">Cash Received</label>
                        <input type="number" id="cashReceived" oninput="calculateTotals()" placeholder="Enter amount..." 
                               class="w-full px-3 py-2 bg-white border-2 border-slate-800 rounded-2xl text-xl font-black text-slate-800 outline-none focus:ring-4 focus:ring-slate-100 transition-all shadow-sm">
                    </div>

                    <div id="balanceBox" class="flex justify-between items-center px-2 mt-1">
                        <span id="balanceLabel" class="text-[10px] font-bold uppercase text-slate-400">Return Change</span>
                        <span id="balanceAmount" class="text-lg font-black text-slate-800">0.00</span>
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-3 mt-6">
                    <button onclick="processCheckout('cash')" class="bg-slate-800 text-white py-4 rounded-2xl shadow-lg hover:bg-slate-900 font-black uppercase text-xs tracking-widest transition active:scale-95 ring-4 ring-slate-50">
                        <i class="fa-solid fa-cash-register mr-2"></i> Cash Sale
                    </button>
                    <button onclick="processCheckout('credit')" class="bg-amber-500 text-white py-4 rounded-2xl shadow-lg hover:bg-amber-600 font-black uppercase text-xs tracking-widest transition active:scale-95 ring-4 ring-amber-50">
                        <i class="fa-solid fa-file-invoice-dollar mr-2"></i> Add to Ledger
                    </button>
                </div>
            </div>
        </div>
...
<script>
    ...
    async function fetchProducts(page = 1) {
        ...
        grid.innerHTML = '<div class="col-span-full py-20 text-center"><i class="fa-solid fa-circle-notch fa-spin text-3xl text-amber-500"></i></div>';
    ...
    function renderGrid(products) {
        ...
        products.forEach(p => {
            const stockColor = p.stock_level < 10 ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-slate-600';
            grid.innerHTML += `
                <div class="product-card group relative p-4 border border-gray-100 rounded-2xl bg-white shadow-sm hover:shadow-xl hover:border-amber-400 transition-all duration-300 cursor-pointer flex flex-col justify-between h-[190px] transform hover:-translate-y-1"
                     onclick="addToCart(${p.id}, '${p.medicine.name}', ${p.sale_price || 0}, '${p.sku}', ${p.stock_level})">
                    <div class="flex justify-between items-start">
                        <span class="px-2 py-1 rounded-lg text-[9px] font-black uppercase ${stockColor}">In Stock: ${p.stock_level}</span>
                        <i class="fa-solid fa-box-open text-gray-200 group-hover:text-amber-500"></i>
                    </div>
                    <div class="mt-2">
                        <h4 class="font-black text-sm text-slate-800 leading-tight line-clamp-2 uppercase">${p.medicine.name}</h4>
                        <p class="text-[9px] font-bold text-amber-500 mt-1 uppercase">CODE: ${p.sku}</p>
                    </div>
                    <div class="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between">
                        <div><span class="text-base font-black text-slate-900 italic"><span class="text-[10px] font-bold text-gray-400 not-italic">PKR</span> ${Number(p.sale_price).toLocaleString()}</span></div>
                        <div class="w-8 h-8 rounded-xl bg-gray-50 group-hover:bg-slate-800 text-gray-300 group-hover:text-white flex items-center justify-center transition-all shadow-sm"><i class="fa-solid fa-plus text-xs"></i></div>
                    </div>
                </div>`;
        });
    }
    ...
    function renderPagination(data) {
        ...
        if (data.last_page > 1) {
            for (let i = 1; i <= data.last_page; i++) {
                const active = i === data.current_page ? 'bg-slate-800 text-white shadow-lg' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200';
    ...
    window.addToCart = function (id, name, price, sku, stock) {
        ...
        if (item) {
            ...
            } else { Swal.fire('Stock Limit', 'Max inventory reached!', 'warning'); }
        } else {
    ...
    function renderCart() {
        ...
        list.innerHTML = cart.length === 0 ? '<div class="text-center py-20 text-gray-300 italic font-bold">POS Ready</div>' : '';
        cart.forEach(item => {
            list.innerHTML += `
                <div class="cart-item flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm transition">
                    <div class="flex flex-col">
                        <p class="font-black text-xs text-slate-800 uppercase tracking-tight">${item.name}</p>
                        <p class="text-[9px] font-bold text-amber-600 uppercase tracking-widest">${item.sku} @ ${item.unitPrice}</p>
                    </div>
    ...
    function calculateTotals() {
        ...
        if (balance >= 0) {
            ...
            balanceElem.className = "text-lg font-black text-slate-800";
        } else {
    ...
    async function processCheckout(method) {
        ...
        try {
            ...
            if (res.success) {
                Swal.fire('Success', 'Transaction Completed & Ledger Updated!', 'success').then(() => location.reload());

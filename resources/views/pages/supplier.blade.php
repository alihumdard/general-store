...
@section('title', 'RetailPro | Supplier Management')

@section('content')
    <main class="overflow-y-auto p-2 sm:p-4 md:p-8 min-h-[calc(100vh-70px)] bg-slate-50 mt-14 sm:mt-0">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 border-b border-slate-200 pb-6">
                <div>
                    ...
                    <p class="text-[10px] text-amber-600 font-bold uppercase tracking-widest mt-2 flex items-center gap-2">
                        ...
                        Directory & Ledger Management
                    </p>
                </div>
                
                <button onclick="openModalForAdd()"
                    class="w-full md:w-auto px-6 py-3 bg-[#0f172a] text-white rounded-2xl shadow-xl hover:bg-slate-800 font-black uppercase text-xs tracking-widest transition active:scale-95 flex items-center justify-center gap-2">
                    <i class="fa-solid fa-plus group-hover:rotate-90 transition-transform"></i> Add New Supplier
                </button>
            </div>

            <div class="bg-white rounded-[30px] shadow-2xl border border-slate-100 p-4 sm:p-8">
                {{-- Filters & Search --}}
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
                    <div class="relative w-full lg:w-1/3">
                        <i class="fa-solid fa-search absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                        <input type="text" id="searchInput" placeholder="Search by name..."
                            class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-sm font-bold focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition shadow-inner"
                            oninput="debounceFetchData()">
                    </div>

                    <div class="w-full lg:w-auto">
                        <select id="balanceFilter" onchange="fetchData()"
                            class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-4 focus:ring-amber-500/10">
                ...
                <div class="hidden md:block overflow-hidden rounded-2xl border border-slate-100 shadow-sm">
                    <table class="w-full border-collapse text-sm">
                        <thead>
                            <tr class="bg-[#0f172a] text-white border-b border-slate-100 uppercase font-black text-[10px] tracking-widest italic">
                        ...
                        <tbody id="suppliersTableBody" class="divide-y divide-slate-50 text-slate-800">
                            {{-- Content via JS --}}
                        </tbody>
                    </table>
                </div>
    ...
    <div id="historyModal" class="fixed inset-0 bg-[#0f172a]/80 hidden justify-center items-center z-[60] p-2 sm:p-4 backdrop-blur-sm transition-all duration-300">
        <div id="historyModalBox" class="bg-white w-full max-w-6xl max-h-[90vh] rounded-[40px] shadow-3xl flex flex-col overflow-hidden border border-slate-100 transform scale-95 opacity-0 transition-all duration-300">
            <div class="p-6 border-b flex justify-between items-center bg-[#0f172a] shrink-0 text-white">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 id="historySupplierName" class="text-lg sm:text-2xl font-black tracking-tighter leading-none italic uppercase text-amber-500">Statement</h2>
                        <span id="historySupplierPhone" class="text-[10px] font-bold text-slate-400 uppercase tracking-widest"></span>
                    </div>
                </div>
            ...
            <div class="flex flex-col lg:flex-row flex-grow overflow-hidden bg-slate-50">
                ...
                <div class="lg:w-2/3 flex-grow overflow-y-auto p-6 border-r border-slate-200 custom-scrollbar bg-white">
                    <h3 class="text-[10px] font-black uppercase text-slate-400 mb-6 tracking-widest italic border-l-4 border-amber-500 pl-3 uppercase">Transaction Manifest</h3>
                    <div id="historyContent" class="overflow-x-auto rounded-3xl border border-slate-100"></div>
                </div>

                {{-- Record Payment --}}
                <div class="lg:w-1/3 p-6 flex flex-col shrink-0 overflow-y-auto">
                    ...
                    <div class="bg-white p-6 rounded-[30px] border border-slate-200 shadow-inner">
                        ...
                        <div class="space-y-4">
                            <select id="manualType" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl font-bold text-sm outline-none">
                                <option value="debit">Payment Sent (Reduce Balance)</option>
                                <option value="credit">Purchase/Credit (Increase Balance)</option>
                            </select>
                            <input type="number" id="manualAmount" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl font-black text-lg outline-none" placeholder="0.00">
        ...
        <div id="supplierModalBox" class="bg-white w-full max-w-2xl max-h-[95vh] overflow-y-auto rounded-[40px] shadow-3xl p-8 transform scale-95 opacity-0 transition-all duration-300 flex flex-col">
            <div class="flex justify-between items-center mb-8 border-b border-slate-100 pb-5">
                <div>
                    ...
                    <p class="text-[10px] font-bold text-amber-600 uppercase tracking-widest mt-1">Directory Metadata</p>
                </div>
            ...
            <form id="supplierForm" onsubmit="handleFormSubmit(event)" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="space-y-1">
                        ...
                        <input type="text" name="supplier_name" required class="w-full px-4 py-3 border border-slate-200 rounded-2xl bg-slate-50 text-sm font-bold outline-none focus:border-amber-500 transition">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Contact Person</label>
                        <input type="text" name="contact_person" class="w-full px-4 py-3 border border-slate-200 rounded-2xl bg-slate-50 text-sm font-bold outline-none focus:border-amber-500 transition">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Phone Number</label>
                        <input type="text" name="phone_number" class="w-full px-4 py-3 border border-slate-200 rounded-2xl bg-slate-50 text-sm font-bold outline-none focus:border-amber-500 transition">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Address</label>
                        <input type="email" name="email" class="w-full px-4 py-3 border border-slate-200 rounded-2xl bg-slate-50 text-sm font-bold outline-none focus:border-amber-500 transition">
                    </div>
                    <div class="sm:col-span-2 p-6 bg-slate-50 rounded-[30px] border border-slate-200">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block uppercase">Opening Balance (PKR)</label>
                        <input type="number" step="0.01" name="balance_due" value="0.00" class="w-full px-5 py-4 border-2 border-slate-200 rounded-2xl bg-white font-black text-2xl text-slate-900 italic outline-none focus:ring-8 focus:ring-amber-50/50 shadow-sm">
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row justify-end gap-3 mt-10 shrink-0">
                    <button type="button" onclick="closeModal()" class="px-8 py-4 bg-white border border-slate-200 rounded-2xl text-slate-400 font-black uppercase text-[10px] tracking-widest">Discard</button>
                    <button type="submit" id="saveSupplierBtn" class="px-10 py-4 bg-[#0f172a] text-[#f59e0b] rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl hover:bg-slate-800 transition active:scale-95">Confirm</button>
                </div>
            </form>
        </div>
    </div>
